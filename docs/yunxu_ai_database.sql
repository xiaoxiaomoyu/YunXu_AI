-- 导出 yunxu_ai 的数据库结构 
DROP DATABASE IF EXISTS `yunxu_ai`; 
CREATE DATABASE IF NOT EXISTS `yunxu_ai`; 
USE `yunxu_ai`; 

-- 导出  表 yunxu_ai.course 结构 
...
-- 正在导出表  yunxu_ai.course 的数据：~7 rows (大约) 
DELETE FROM `course`; 
...
-- 导出  表 yunxu_ai.course_reservation 结构 
...
-- 正在导出表  yunxu_ai.course_reservation 的数据：~0 rows (大约) 
...
-- 正在导出表  yunxu_ai.school 的数据：~0 rows (大约) 
DELETE FROM `school`; 
